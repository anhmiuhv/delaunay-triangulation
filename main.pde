...
void draw() {
  ...
  if(step2)     
    {
    fill(yellow);
    if(live) {M.MatchAndPair(0);}//computeO();} // **02 implement it in Mesh
    if(showEdges) 
      {
      fill(yellow); 
  ...
  if(step4)
    {
    for(int i=0; i<1; i++) M.smoothenInterior(); // **04 implement it in Mesh
    M.writeVerticesTo(R);  
    }
 ...
 // **05 implement corner operators in Mesh
  ...
  int line=0;
  scribeHeader(" Project 3 for Rossignac's 2018 Graphics Course CS3451 / CS6491 by Jeevitesh Juneja ",line++);
